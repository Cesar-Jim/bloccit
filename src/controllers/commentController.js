...
module.exports = {
  create(req, res, next) {
    ...
    if (authorized) {
      ...
      // Call createComment, passing in newComment.
      commentQueries.createComment(newComment, (err, comment) => {
        // redirect the user to the same place regardless of the outcome, so we 
  ...
  // The destroy action passes the request to the deleteComment method to determine if we should be deleted.
